...
function displayFloodPercent(camera: CameraWithPrediction): number {
    if (camera.prediction && typeof camera.prediction.probabilities?.flooded === 'number') {
        const v = camera.prediction.probabilities.flooded
        const clamped = Math.min(100, Math.max(0, v))
        return Number(clamped.toFixed(2))
    }
    return camera.flood_percentage
}
</script>
    ...
    <div v-if="camera" class="grid justify-center">
        ...
        <div class="mt-5 hidden items-center justify-between lg:flex">
            ...
            <div class="hidden gap-5 lg:flex">
                ...
                <button
                    class="rounded-lg bg-blue-500 px-10 py-2 font-semibold text-white shadow-xl"
                >
