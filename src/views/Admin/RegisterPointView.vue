...
import { MapboxComp, BaseForm } from '@/components'
...
<template>
  ...
  >
    <div class="flex flex-col rounded-4xl border border-[#DCDCDC] p-5 dark:border-[#24415D] md:p-7">
      <div>
        <h1 class="text-2xl font-semibold md:text-3xl">Cadastrar ponto de alagamento</h1>
        ...
        <li v-for="step in 3" :key="step">
          ...
          >
            <span
              ...
              "
            >
              <p :class="[currentStep > step ? 'material-symbols-outlined text-lg' : '']">
                {{ currentStep > step ? 'check' : step }}
              </p>
            </span>
            <h4 class="hidden text-xs font-semibold sm:block">
      ...
      <div class="mx-auto">
        <BaseForm :form-fields="loginFields" button-text="Definir risco" @submit="handleSubmit" />
      </div>
    </div>
    <!-- <div
